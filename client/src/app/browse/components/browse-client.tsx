...
import { getBgcs, getGcfs, getSamples, getTaxonomy } from "@/services/api";
...
export function BrowseClient() {
  ...
  const [activeTab, setActiveTab] = useState("bgcs");
        ...
        <Table>
          ...
          <TableBody>
            {loading ? (
              ...
              (type === 'BGC' && bgcData.data.length === 0) ||
              (type === 'GCF' && gcfData.data.length === 0) ? (
                <TableRow>
                ...
                // Otherwise show skeleton for now (would be real data in production)
                Array.from({ length: 5 }).map((_, i) => (
                  <TableRow key={i}>
                    <TableCell><Skeleton className="h-4 w-24" /></TableCell>
                    <TableCell><Skeleton className="h-4 w-32" /></TableCell>
